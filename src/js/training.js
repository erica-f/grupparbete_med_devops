...
async function getData() {
  let sessionExercises =
    JSON.parse(sessionStorage.getItem("exercisesList")) || [];
  ...
  let filteredList = filterExerciseList(
    exerciseList,
    exerciseSettings.children,
    exerciseSettings.equipment,
    exerciseSettings.gym,
  );
  let selectedExercises = sessionExercises;
  //Saving exercises for the same session
  if (sessionExercises.length < 1) {
    selectedExercises = getRandomExercises(
      filteredList,
...
function populateWorkout(exerciseId, selectedExercise) {
  ...
  sessionStorage.setItem(
    "exercisesList",
    JSON.stringify(settings.selectedExercises),
  );

  for (let i = 0; i < noOfCards; i++) {
    let repAmount = reps.find(
      (amount) => amount.exercise_id === selectedExercises[i].id,
    );
    let timeOrRep = "reps";
  ...
  document.getElementById("total-exercises").textContent =
    settings.totalNumberOfExercises;
  if (exerciseId != null) {
...
function markExerciseCompleted(e) {
  ...
  sessionStorage.setItem(
    "completedExercises",
    JSON.stringify(completedExercisesList),
  );
  updateProgress();
}
function updateProgress() {
  ...
  let percentDone = Math.round(
    (completedExercises.length / totalNumberOfExercises) * 100,
  );
